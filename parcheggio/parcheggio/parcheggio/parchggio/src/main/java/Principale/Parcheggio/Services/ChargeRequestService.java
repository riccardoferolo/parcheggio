...
public class ChargeRequestService {
    ...
    @Transactional
    public ChargeRequest createChargeRequest(String nome, LocalDate giorno, Time ora, Time durata, Integer Percentuale, Time oraFine) {
        try {
            ...
            reservationService.addReservation(user.getId(),chargeRequest.getId(), payment);
    ...
    public boolean isReservationAvailable(LocalDate giorno, Time ora, Time oraFine) {
        ...
        /*
        // Query JPQL senza funzioni del database
        String jpql = "SELECT COUNT(r) FROM Reservation r " +
                "WHERE r.chargeRequest.Giorno = :giorno " +
                "AND (r.chargeRequest.Ora BETWEEN :oraInizio AND :oraFine " +
                "OR :oraInizio BETWEEN r.chargeRequest.Ora AND :calcolatoOraFine " +
                "OR :oraFine BETWEEN r.chargeRequest.Ora AND :calcolatoOraFine)";
         */

        // Calcola l'ora di fine basandosi sull'ora di inizio e la durata
        //Time oraFine = Time.valueOf(ora.toLocalTime().plusSeconds(durata.getTime() / 1000));
        ...
        String jpql = "SELECT COUNT(r) " +
                ...
                "WHERE r.chargeRequest.Giorno = :giorno " +
                "AND (" +
                "  r.chargeRequest.Ora < :oraFine AND " +   // La prenotazione esistente inizia prima della fine della nuova
                "  r.chargeRequest.OraFine > :oraInizio" + // La prenotazione esistente termina dopo l'inizio della nuova
                ")";

        Query query = entityManager.createQuery(jpql);
        query.setParameter("giorno", giorno);
        query.setParameter("oraInizio", ora); // Ora di inizio della nuova prenotazione
        query.setParameter("oraFine", oraFine);

        // Esecuzione della query per ottenere il numero di prenotazioni sovrapposte
