...
public class ChargeRequestController {
    ...
    public ResponseEntity<?> createChargeRequest(@RequestBody Map<String, Object> requestBody) {
        System.out.println("Richiesta ricevuta con dati: " + requestBody);
        try {
            ...
            Integer percentuale = (Integer) requestBody.get("Percentuale"); // Deve essere un intero
            ...
            ChargeRequest chargeRequest = chargeRequestService.createChargeRequest(nome, data, ora, durata, percentuale,oraFine);
    ...
    public String calcola(Integer percentuale) {
        Integer p = percentuale * 180;
        int ora = 0;
        double app;
        int minuti;
        if(p < 3600){
            app = p / 60;
            String numberAsString = String.valueOf(app);
            String leftPart = numberAsString.split("\\.")[0];
            minuti = Integer.parseInt(leftPart);
        }
        else{
            while(p >= 3600){
                p = p - 3600;
        ...
        if(ora < 10)
        return "0" + ora + ":" + minuti + ":00";
        else
            return ora + ":" + minuti + ":00";
    }
